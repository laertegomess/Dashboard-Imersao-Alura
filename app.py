...
#datafrane principal é filtrado com base nas seleções feitas na barra lateral. 
df_filtrado = df[
    (df['ano'].isin(anos_selecionados)) &
    (df["senioridade"].isin(senioridades_selecionadas)) &
    (df["contrato"].isin(contratos_selecionados)) &
    (df["tamanho_empresa"].isin(tananhos_selecionados))
]
...
st.subheader("Métricas gerais (Salário anual em USD)")
    ...
    if not df_filtrado.empty:
         ...
         salario_maxino = df_filtrado['usd'].max()
...
#Análises Visuals com Plotly 
st.subheader("Gråficos")

col_graf1, col_graf2 = st.colunns(2)
...
if not df_filtrado.empty: 
    ...
    grafico_cargos = px.bar(
        ...
        labels={"usd" : "Mēdia salarial anual (USD)", "cargo": "cargo"}
    )
    ...
    st.warning("Nenhum dado para exibir no grofico de cargos.")
...
with col_graf2:
    ...
    st.plotly_chart(grafico_hist, use_confainer width=True)
else:
    st.warning("Nenhum dado para exibir no gráfico de distribuição.")

 col_graf3, col graf4 = st.columns(2)

with col_graf3:
    if not df_filtrado.empty:
    remoto_contagem = df_filtrado["remoto"].value_counts().reset_index()
    remoto_contagem.columns = ["tipo_trabalho", "quantidade"] 
    gratico_remoto = px.pie(
    remoto_contagen, 
    names="tipo_trabalho"
    values="quantidade"
...
# Ajuste de nomes de colunas para evitar erros
...
# --- Gráfico 2: Histograma de salários
with col_graf2:
    if not df_filtrado.empty:
        grafico_hist = px.histogram(
            df_filtrado,
...
# --- Gráfico 3: Trabalho Remoto / Híbrido / Presencial
with col_graf3:
    if not df_filtrado.empty:
        remoto_contagem = df_filtrado["remoto"].value_counts().reset_index()
        remoto_contagem.columns = ["tipo_trabalho", "quantidade"]
...
# --- Gráfico 4: Mapa Salarial (Cientistas de Dados)
with col_graf4:
    if not df_filtrado.empty:
        df_ds = df_filtrado[df_filtrado["cargo"] == "Data Scientist"]
